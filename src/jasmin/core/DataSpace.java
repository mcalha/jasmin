 ...
 */
public class DataSpace implements Cloneable {
    ...
    /**
     * the Parser working with this DataSpace. Needed to call some of its
     ...
     */
    private static String[] registers = {"RAX","EAX", "AX", "AL", "AH", "RBX","EBX", "BX", "BL", "BH", "RCX","ECX", "CX",
        "CL", "CH", "RDX","EDX", "DX",
        "DL", "DH", "RSI","ESI", "SI", "RDI","EDI", "DI", "RSP","ESP", "SP", "RBP","EBP", "BP", "EIP", "R8", "R8D", "R8W",
        "R8B","R9", "R9D", "R9W", "R9B","R10", "R10D", "R10W", "R10B","R11", "R11D", "R11W", "R11B","R12", "R12D", "R12W", "R12B",
     ...
     */
    public Address RAX, EAX, AX, AH, AL, RBX, EBX, BX, BH, BL, RCX, ECX, CX, CH, CL, RDX, EDX, DX, DH, DL, RSI,ESI, SI, RDI, EDI,
            DI, RSP, ESP, SP, RBP, EBP, BP, R8, R8D, R8W, R8B,R9, R9D, R9W, R9B,R10, R10D, R10W, R10B,R11, R11D, R11W, R11B,R12, R12D,
     ...
     * @return the address where the stackpointer points to. for highlighting in
     * the GUI
     */
    ...
    /**
     * Checks whether an access to an invalid (out-of-range) address occurred.
     * The corresponding flag must be reset manually afterwards!
     *
    ...
    /**
     * set Instruction Pointer, which in this case is not an Address, but rather
     * a line number for the interpreter
     *
    ...
    /**
     * get Instruction Pointer, which in this case is not an Address, but rather
     * a line number for the interpreter
     *
    ...
    @Override
    public DataSpace clone() throws CloneNotSupportedException {
        try{
            DataSpace res = (DataSpace) super.clone();
            return res;
     ...
     * initiate memory
     *
     * @param size which size the memory array has. The smallest multiple of 4
     * greater than size will be taken
    ...
    /**
     * initialize the register array, the hashtable for the registers, the
     * RegisterSets for the GUI and the SmallArguments referring to the
     * registers
     */
    private void initRegisters() {
        ...
        RAX = reg.constructAddress("RAX", regtable);
        ...
        RBX = reg.constructAddress("RBX", regtable);
        ...
        RCX = reg.constructAddress("RCX", regtable);
        ...
        RDX = reg.constructAddress("RDX", regtable);
        ...
        RSI = reg.constructAddress("RSI", regtable);
        ESI = reg.constructAddress("ESI", regtable);
        SI = reg.constructAddress("SI", regtable);
        RDI = reg.constructAddress("RDI", regtable);
        EDI = reg.constructAddress("EDI", regtable);
        DI = reg.constructAddress("DI", regtable);
        RSP = reg.constructAddress("RSP", regtable);
        ESP = reg.constructAddress("ESP", regtable);
        SP = reg.constructAddress("SP", regtable);
        RBP = reg.constructAddress("RBP", regtable);
        EBP = reg.constructAddress("EBP", regtable);
        BP = reg.constructAddress("BP", regtable);
        EIP = reg.constructAddress("EIP", regtable);
        R8 = reg.constructAddress("R8", regtable);
        R8B = reg.constructAddress("R8B", regtable);
        R8W = reg.constructAddress("R8W", regtable);
        R8D = reg.constructAddress("R8D", regtable);
        R9 = reg.constructAddress("R9", regtable);
        R9B = reg.constructAddress("R9B", regtable);
        R9W = reg.constructAddress("R9W", regtable);
        R9D = reg.constructAddress("R9D", regtable);
        R10 = reg.constructAddress("R10", regtable);
        R10B = reg.constructAddress("R10B", regtable);
        R10W = reg.constructAddress("R10W", regtable);
        R10D = reg.constructAddress("R10D", regtable);
        R11 = reg.constructAddress("R11", regtable);
        R11B = reg.constructAddress("R11B", regtable);
        R11W = reg.constructAddress("R11W", regtable);
        R11D = reg.constructAddress("R11D", regtable);
        R12 = reg.constructAddress("R12", regtable);
        R12B = reg.constructAddress("R12B", regtable);
        R12W = reg.constructAddress("R12W", regtable);
        R12D = reg.constructAddress("R12D", regtable);
        R13 = reg.constructAddress("R13", regtable);
        R13B = reg.constructAddress("R13B", regtable);
        R13W = reg.constructAddress("R13W", regtable);
        R13D = reg.constructAddress("R13D", regtable);
        R14 = reg.constructAddress("R10", regtable);
        R14B = reg.constructAddress("R10B", regtable);
        R14W = reg.constructAddress("R10W", regtable);
        R14D = reg.constructAddress("R10D", regtable);
        R15 = reg.constructAddress("R10", regtable);
        R15B = reg.constructAddress("R10B", regtable);
        R15W = reg.constructAddress("R10W", regtable);
        R15D = reg.constructAddress("R10D", regtable);

        registerSets = new RegisterSet[17];
        registerSets[0] = new RegisterSet(AL, AH, AX, EAX, RAX, "AL", "AH", "AX", "EAX", "RAX");
        registerSets[1] = new RegisterSet(BL, BH, BX, EBX, RBX, "BL", "BH", "BX", "EBX", "RBX");
        registerSets[2] = new RegisterSet(CL, CH, CX, ECX,RCX, "CL", "CH", "CX", "ECX", "RCX");
        registerSets[3] = new RegisterSet(DL, DH, DX, EDX,RDX, "DL", "DH", "DX", "EDX", "RDX");
        registerSets[4] = new RegisterSet(null, null, SI, ESI,RSI, "", "", "SI", "ESI", "RSI");
        registerSets[5] = new RegisterSet(null, null, DI, EDI,RDI, "", "", "DI", "EDI", "RDI");
        registerSets[6] = new RegisterSet(null, null, SP, ESP,RSP, "", "", "SP", "ESP", "RSP");
        registerSets[7] = new RegisterSet(null, null, BP, EBP,RBP, "", "", "BP", "EBP","RBP");
        registerSets[8] = new RegisterSet(null, null, null, EIP,null, "", "", "", "EIP","");
        registerSets[9] = new RegisterSet(R8B, null, R8W,R8D, R8, "R8B", " ", "R8W", "R8D","R8");
        registerSets[10] = new RegisterSet(R9B, null, R9W,R9D, R9, "R9B", " ", "R9W", "R9D","R9");
        registerSets[11] = new RegisterSet(R10B, null, R10W,R10D, R10, "R10B", " ", "R10W", "R10D","R10");
        registerSets[12] = new RegisterSet(R11B, null, R11W,R11D, R11, "R11B", " ", "R11W", "R11D","R11");
        registerSets[13] = new RegisterSet(R12B, null, R12W,R12D, R12, "R12B", " ", "R12W", "R12D","R12");
        registerSets[14] = new RegisterSet(R13B, null, R13W,R13D, R13, "R13B", " ", "R13W", "R13D","R13");
        registerSets[15] = new RegisterSet(R14B, null, R14W,R14D, R14, "R14B", " ", "R14W", "R14D","R14");
        registerSets[16] = new RegisterSet(R15B, null, R15W,R15D, R15, "R15B", " ", "R15W", "R15D","R15");
...
//System.out.println("DataSpace - putInteger - memory.set");
    ...
    /**
     * puts a given value into the specified location, accepting binary and
     * hexadecimal Strings. utilizes the functionality of Parser.hex2dec()
     *
    ...
    /**
     * returns a value from memory which should be interpreted as signed,
     * therefore the native type casting of Java is used
     *
     * @param address the address of the (first) memory cell to retrieve
     * @param size the number of memory cells to retrieve
     * @return a long value from the specified location
    ...
    /**
     * returns a register's value, which should be interpreted as signed,
     * therefore the native type casting of Java is used
     *
     ...
     * @return a String containing the value of the register regx in the given
     * type
     */
    ...
    /**
     * set the dirty flag for a register or memory location. Remember that the
     * dirty flag for memory locations is set automatically upon write access.
     *
     ...
     * @param item
     */
    public void updateForCopy(DataSpace item) {
        this.memory = item.memory;
        this.reg = item.reg;
        this.regInfo = item.regInfo;
        this.MEMSIZE = item.MEMSIZE;
        this.memInfo = item.memInfo;
        this.variables = item.variables;
        this.constants = item.constants;
        this.REGSIZE = item.REGSIZE;
        this.fCarry = item.fCarry;
        this.fOverflow = item.fOverflow;
        this.fSign = item.fSign;
        this.fZero = item.fZero;
        this.fParity = item.fParity;
        this.fAuxiliary = item.fAuxiliary;
        this.fTrap = item.fTrap;
        this.fDirection = item.fDirection;
        this.nextReservableAddress = item.nextReservableAddress;
    }
    ...
    // SUPPORTING METHODS
    /**
     * returns the Address referring to the register specified by the given
     * String
     *
    ...
    /**
     * @param a an Argument referring to a register or memory cell about which
     * information is to be retrieved
     * @param writeAccess pass true to create the MemCellInfo object if
     * necessary
     * @return the MemCellInfo object associated with the memory cell with the
     * specified index
     */
...
//System.out.println("DataSpace - memInfoDelete - regInfo index:" + index + " removed:" + ret);
    ...
    /**
     * returns the register matching the given register with the specified size,
     * e.g. passing EAX and 2 will return AX. (!) Be sure to pass one of
     * DataSpace's registers, such as dataspace.EAX, not a newly created one!
     *
    ...
    /**
     * @return an array of type RegisterSet which contains the String names of
     * the registers which belong together.
     */
     ...
     * @param a an argument describing the register/memory address where the
     * value is to be stored
     * @param memCellInfo the MemCellInfo object that is to be stored. Pass null
     * if none is needed. general purpose method to store a value
     */
...
//System.out.println("DataSpace - put - memInfoPut");
    ...
    /**
     * retrieves the value of a non-static argument such as a register (as
     * opposed to, e.g., an immediate)
     *
     * @param a the argument specifying the register/memory location whose value
     * is to be read
    ...
    /**
     * retrieves the value of a static argument, such as an immediate, or a
     * label
     *
    ...
    /**
     * allocates/reservates memory: a specified block size in bytes, multiplied
     * by a given number of blocks. Currently does not check whether the
     * starting address is a multiple of the requested size, e.g. it is
     ...
     * @return true if the String represents a registered variable, false
     * otherwise
     */
