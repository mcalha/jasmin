...
public class Registers implements Cloneable {
    //+8 registos adicionais para o modo 64-bit (R8, …, R15)
    int NUMREG = 17; // A, B, C, D, SI, DI, SP, BP, IP
    ...
    public Registers clone() throws CloneNotSupportedException {
        try{
            Registers res = (Registers) super.clone();
            return res;
    ...
    public Address constructAddress(String registerName, Hashtable<String, Address> registerTable) {
        ...
        // determine the size
        if(registerName.matches("R[A-Z][A-Z].|R[89]|R1[0-9]")){
            register.type = Op.R64;
            register.size = 8;
            register.mask = 0xFFFFFFFFFFFFFFFFL;
            register.rshift = 0;
        } else if (registerName.matches("E..|...D|..D")) {
            register.type = Op.R32;
            register.size = 4;
            register.mask = 0xFFFFFFFFL;
            register.rshift = 0;
        } else if (registerName.matches(".[XIP]|...W|..W")) {
            ...
            register.rshift = 8;
        } else if (registerName.matches(".L|...B|..B")) {
            register.type = Op.R8;
            register.size = 1;
            register.mask = 0xFFL;
            register.rshift = 0;
        ...
        if (registerName.matches("[RE]?A[HLX]")) {
            register.address = 0;
        } else if (registerName.matches("[RE]?B[HLX]")) {
            register.address = 1;
        } else if (registerName.matches("[RE]?C.")) {
            register.address = 2;
        } else if (registerName.matches("[RE]?D[HLX]")) {
            register.address = 3;
        } else if (registerName.matches("[RE]?SI")) {
            register.address = 4;
        } else if (registerName.matches("[RE]?DI")) {
            register.address = 5;
        } else if (registerName.matches("[RE]?SP")) {
            register.address = 6;
        } else if (registerName.matches("[RE]?BP")) {
            register.address = 7;
        } else if (registerName.matches("E?IP")) {
            register.address = 8;
        } else if (registerName.matches("R8[WBD]?")) {
            register.address = 9;
        } else if (registerName.matches("R9[WBD]?")) {
            register.address = 10;
        } else if (registerName.matches("R10[DWB]?")) {
            register.address = 11;
        } else if (registerName.matches("R11[WDB]?")) {
            register.address = 12;
        } else if (registerName.matches("R12[WBD]?")) {
            register.address = 13;
        } else if (registerName.matches("R13[WBD]?")) {
            register.address = 14;
        } else if (registerName.matches("R14[WBD]?")) {
            register.address = 15;
        } else if (registerName.matches("R15[WBD]?")) {
            register.address = 16;
        }
